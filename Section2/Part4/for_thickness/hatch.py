...
class VerticalHatch(HatchPatternBase):
    def __init__(self, hatch, density, thickness=1.0):
        self.num_lines = int((hatch.count('|') + hatch.count('+')) * density)
    ...
    def set_vertices_and_codes(self, vertices, codes):
        ...
        for i in range(self.num_lines):
            ...
            vertices[i*4 + 0, 0] = steps[i] - half_thickness
            vertices[i*4 + 0, 1] = 0.0
            vertices[i*4 + 1, 0] = steps[i] + half_thickness
            vertices[i*4 + 1, 1] = 0.0
            vertices[i*4 + 2, 0] = steps[i] + half_thickness
            vertices[i*4 + 2, 1] = 1.0
            vertices[i*4 + 3, 0] = steps[i] - half_thickness
            vertices[i*4 + 3, 1] = 1.0
...
def get_path(hatchpattern, density=6, thickness=1.0):
    """
    ...
    patterns = [VerticalHatch(hatchpattern, density, thickness)]
